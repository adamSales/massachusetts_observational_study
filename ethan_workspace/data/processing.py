...
modeling_data['inverse_mastery_speed'] = modeling_data.apply(
    lambda x: 1 / x['problems_completed'] if x['assignment_completed'] else 0, axis=1)
...
input_data['time_since_last_assignment_start_cluster'] = GaussianMixture(n_components=clusters).fit(times).predict(
    times)
...
categorical_features.append('no_problem_statistics')

# Replace NaN categorical features with -1
input_data[categorical_features] = input_data[categorical_features].fillna(-1)

# Add the previous assignments and target sequence to the training data
...
def process_input(df_list, max_sequence_length, one_hot_encoder=None, normalizer=None):
...
for train_index, test_index in GroupKFold(N_SPLITS).split(modeling_data, groups=modeling_data['class_id']):
    ...
    training_input, input_one_hot_encoder, input_normalizer = process_input(
        modeling_data.iloc[train_index]['previous_assignments'].tolist(), MSL)
    ...
    testing_input, _, _ = process_input(modeling_data.iloc[test_index]['previous_assignments'].tolist(), MSL,
                                        input_one_hot_encoder, input_normalizer)
    ...
    np.save(f'processed_data_{MSL}/training_input_{count}.npy', training_input)
    ...
    np.save(f'processed_data_{MSL}/testing_input_{count}.npy', testing_input)

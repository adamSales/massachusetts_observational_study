...
for i in range(N_SPLITS):
    ...
    # Combine both inputs
    prior_training_input = np.expand_dims(prior_training_input, axis=1)
    prior_training_input = np.repeat(prior_training_input, recurrent_training_input.shape[1], axis=1)
    recurrent_training_input = np.concatenate([recurrent_training_input, prior_training_input], axis=2)
    prior_testing_input = np.expand_dims(prior_testing_input, axis=1)
    prior_testing_input = np.repeat(prior_testing_input, recurrent_testing_input.shape[1], axis=1)
    recurrent_testing_input = np.concatenate([recurrent_testing_input, prior_testing_input], axis=2)

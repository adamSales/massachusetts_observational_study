...
for train_index, test_index in GroupKFold(N_SPLITS).split(modeling_data, groups=modeling_data['class_id']):
    ...
    training_input, input_one_hot_encoder, input_normalizer = process_input(modeling_data.iloc[train_index]['previous_assignments'].tolist(), MSL)
    ...
    testing_input, _, _ = process_input(modeling_data.iloc[test_index]['previous_assignments'].tolist(), MSL, input_one_hot_encoder, input_normalizer)
